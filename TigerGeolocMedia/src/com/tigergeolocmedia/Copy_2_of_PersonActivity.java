...
public class Copy_2_of_PersonActivity extends ParentMenuActivity {
	...
	private void testRetrofitFull() {
		...
		Observable<Person> o2 = userList.flatMap(new Func1<List<Person>, Observable<? extends Person>>() {
			...
			public Person call(Person person) {
				...
				Observable<Person> personComplete = retrieveUsersService.getUser(person.getLogin()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread());
				Observer<Person> localObserver = new Observer<Person>() {
				...
				personComplete.subscribe(localObserver);
